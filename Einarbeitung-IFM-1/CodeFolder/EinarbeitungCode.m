...
% For the MP rule: E_(n+1) - E_n plot:
EnDiffMP = zeros(size(MPEnergy,1)-1, 2);
for t=2:size(MPEnergy,1)
    EnDiffMP(t-1, 2) = MPEnergy(t,2)-MPEnergy(t-1,2);
    EnDiffMP(t-1,1) = MPEnergy(t,1);
end

figure(6)
plot(EnDiffMP(:,1), EnDiffMP(:,2))
xlabel("Time [s]")
...
cleanfigure;
matlab2tikz('height', '\figH', 'width', '\figW', 'filename', ['Figure6', '.tex'], 'showInfo', false, 'floatformat', '%.4g');
