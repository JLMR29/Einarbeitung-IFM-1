...
\DeclareBoolOption[false]{widemargin}
%biblography
...
%right margin
\ififmdocument@widemargin%
    \setlength{\ifm@right}{30mm}
    \setlength{\ifm@marginpar}{28mm}
\else
    \setlength{\ifm@right}{20mm}
    \setlength{\ifm@marginpar}{18mm}
\fi
...
\def\@geomoptions{a4paper,left=20mm,right=\ifm@right,top=20mm,bottom=11mm,foot=\ifm@foot,head=\ifm@head,marginparwidth=\ifm@marginpar,marginparsep=2mm,headsep=\ifm@headsep,\ifm@inclheadfoot}
...
\ExecuteBibliographyOptions{sorting=nyt,sortcites=true,maxbibnames=10,maxcitenames=2,uniquename=false}
...
\DeclareFieldFormat[article]{pages}{#1\isdot}%no pp.
